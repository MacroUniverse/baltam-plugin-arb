...
if max(abs((hypergeom(0.1, -0.2+0.1i, [0.3,-0.1,0.2]) - val)./val)) > eps
	   error('failed!');
end
...
if max(abs((gammaC([2.456; -1.324+0.1i]) - val)./val)) > eps
	 error('failed!');
end
...
val = log([1.289458075329835
	 2.838662274628721 + 0.715540219254106i]);
if max((abs(lgamma([2.456; -1.324+0.1i]) - val)./val)) > 2*eps
	 error('failed!');
end
...
%% 测试 BigFloat 浮点数
digits(32);
d = BigFloat("1234567890.223456789032345678904234567890");
e = BigFloat("8765432109.776543210967654321095765432109");
f = d + e;
f
